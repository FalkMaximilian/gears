...
- **`engine.rs`** — `WorkflowEngineBuilder` + `WorkflowEngine`. Manages workflow/activity registration, dispatch loop, recovery on startup, concurrency via semaphore, `cancel_workflow()`, `list_runs()`, `get_run_events()`, `workflow_names()`, `activity_names()`, `activity_info_list()`, `workflow_info_list()`, cleanup policy (`cleanup_policy(CleanupPolicy)`), and typed variants (`start_workflow_typed`, `get_run_result`, `get_run_result_typed`). `ActivityInfo` struct carries per-activity metadata (name, max_attempts, retry_base_delay_ms, timeout_ms). `WorkflowInfo` struct carries per-workflow metadata (name, effective retention_days); returned by `workflow_info_list()`. `RetentionPolicy` controls automatic deletion of terminal runs: global via `retention_days(u32)` on the builder; per-workflow via `Workflow::retention() -> Option<Duration>` (overrides global when `Some`). A background hourly Tokio task runs `run_pruning_pass` and deletes expired runs; it starts only when at least one retention period is configured and shuts down cleanly via `EngineHandle::shutdown()`.
- **`api.rs`** — `management_router()` returns an Axum `Router<Arc<WorkflowEngine>>` with REST endpoints for all engine management operations. Mount it with `.nest("/api", management_router())`. Endpoints: `GET /runs`, `POST /runs` (start a run; body: `{workflow_name, input}`; returns `{run_id}`), `GET /runs/{id}`, `GET /runs/{id}/events` (full ordered event log as JSON array), `POST /runs/{id}/cancel`, `GET /schedules`, `POST /schedules`, `DELETE /schedules/{name}`, `POST /schedules/{name}/pause`, `POST /schedules/{name}/resume`, `GET /workflows` (returns `WorkflowInfo` objects with name + effective `retention_secs`, not just names), `GET /activities` (returns `ActivityInfo` objects with retry config), `POST /runs/prune` (triggers a pruning pass immediately), `GET /openapi.json` (OpenAPI 3.1 spec). All request/response types carry `#[derive(utoipa::ToSchema)]`; the spec is generated at compile time via `GearsApiDoc` and served on each request. `openapi_spec() -> utoipa::openapi::OpenApi` is also exported from the library root for programmatic use.
...
- **`src/bin/gears-ctl/`** — Standalone TUI controller binary (`gears-ctl`). Connects to the management API over HTTP. Four files: `main.rs` (event loop, terminal setup), `app.rs` (state + actions), `client.rs` (reqwest API client), `ui.rs` (ratatui rendering). Three tabs: **Runs** (list with status/workflow/id/duration/updated), **Schedules** (list with cron/status/last-fired), **Registered** (split view: workflows left with name + retention label, activities with retry config right). Key bindings:
